...
Template.main.onRendered ->
  ...
  getScreens = ->
    screenHeight = do $(window).height - 4.375 * $('body').css('font-size').slice 0, -2
    $('.slogan-box').outerHeight screenHeight
    $('.assortment').outerHeight screenHeight
    $('.map').outerHeight screenHeight
    screens = []
    $('.j-screen').each ->
      $self = $(@)
